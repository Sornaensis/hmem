...
userRowBySubjectStatement = Statement.Statement sql encoder decoder True
  where
    sql = "SELECT id, can_create_workspace, is_superadmin \
          \FROM users WHERE auth_subject = $1"
    encoder = Enc.param (Enc.nonNullable Enc.text)
    decoder = Dec.rowMaybe userRowDecoder

insertUserStatement :: Statement.Statement (Text, Maybe Text, Maybe Text, Bool, Bool) UserRow
insertUserStatement = Statement.Statement sql encoder decoder True
  where
    sql = "INSERT INTO users (auth_subject, email, display_name, can_create_workspace, is_superadmin) \
          \VALUES ($1, $2, $3, $4, $5) \
          \RETURNING id, can_create_workspace, is_superadmin"
    encoder =
      contramap (\(a,_,_,_,_) -> a) (Enc.param (Enc.nonNullable Enc.text)) <>
      contramap (\(_,b,_,_,_) -> b) (Enc.param (Enc.nullable Enc.text)) <>
      contramap (\(_,_,c,_,_) -> c) (Enc.param (Enc.nullable Enc.text)) <>
      contramap (\(_,_,_,d,_) -> d) (Enc.param (Enc.nonNullable Enc.bool)) <>
      contramap (\(_,_,_,_,e) -> e) (Enc.param (Enc.nonNullable Enc.bool))
    decoder = Dec.singleRow userRowDecoder

updateUserStatement :: Statement.Statement (UUID, Maybe Text, Maybe Text, Maybe Bool, Maybe Bool) UserRow
updateUserStatement = Statement.Statement sql encoder decoder True
  where
    ...
    encoder =
      ...
      contramap (\(_,b,_,_,_) -> b) (Enc.param (Enc.nullable Enc.text)) <>
      contramap (\(_,_,c,_,_) -> c) (Enc.param (Enc.nullable Enc.text)) <>
      contramap (\(_,_,_,d,_) -> d) (Enc.param (Enc.nullable Enc.bool)) <>
      contramap (\(_,_,_,_,e) -> e) (Enc.param (Enc.nullable Enc.bool))
    decoder = Dec.singleRow userRowDecoder
